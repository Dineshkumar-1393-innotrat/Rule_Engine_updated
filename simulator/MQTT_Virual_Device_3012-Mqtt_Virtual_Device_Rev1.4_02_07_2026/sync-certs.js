...
// ── 3. Patch .env with device identifiers and broker ─────────────────────
if (bundle.config) {
  const { vin, imei, tboxSerial, broker } = bundle.config;
  ...
  console.log(`✓  Updated .env  →  VIN=${vin}  IMEI=${imei}  TBOX_SERIAL=${tboxSerial}  BROKER=${broker || 'not set'}`);
}
