...
async function sendCommandResponse(client) {
  ...
  const obj = {
    ...
    subtype: protoObjects.commandResponseSubType.values.RemoteBlinkerOffCommandResponse,
    ...
    commandResponsePayload: {
      remoteBlinkerOffCommandResponsePayload: {
        commandStatus: protoObjects.CommandStatus.values.failure,
        returnFailureCodes:protoObjects.RemoteCommandFailureCodes.values.invalidRemoteFailureCode,
      //  BlinkerOffEventStatus: protoObjects.RemoteBlinkerEventType.values.TurnBlinkers_on
      }
    }
  };
...
async function startCommandResponse() {
  ...
  protoObjects = {
    ...
    RemoteBlinkerEventType: root.lookupEnum("stla.cvip.jeep.RemoteBlinkerEventType"),
    RemoteCommandFailureCodes: root.lookupEnum("stla.cvip.jeep.RemoteCommandFailureCodes")
  };

  console.log(' Protobuf LOADED');
console.log("protoObjects:", Object.keys(protoObjects));
  ...
  const client = mqtt.connect(CONFIG.BROKER, {
    ...
    // key: fs.readFileSync('./certs/ping-key.pem'),
     ca: fs.existsSync('./certs/cacert.pem') ? fs.readFileSync('./certs/cacert.pem') : (fs.existsSync('./certs/ca.crt') ? fs.readFileSync('./certs/ca.crt') : fs.readFileSync('./T123ZTZT867677788_Modified/T123ZTZT867677788_Modified/cacert.pem')),
        cert: fs.existsSync('./certs/client.pem') ? fs.readFileSync('./certs/client.pem') : (fs.existsSync('./certs/client.crt') ? fs.readFileSync('./certs/client.crt') : fs.readFileSync('./T123ZTZT867677788_Modified/T123ZTZT867677788_Modified/client.pem')),
        key: fs.existsSync('./certs/device-key.key') ? fs.readFileSync('./certs/device-key.key') : (fs.existsSync('./certs/client.key') ? fs.readFileSync('./certs/client.key') : fs.readFileSync('./T123ZTZT867677788_Modified/T123ZTZT867677788_Modified/device-key.key')),
    protocolVersion: 4
  });
