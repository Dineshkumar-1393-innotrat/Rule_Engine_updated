...
const CONFIG = {
  ...
  VIN: process.env.VIN,
  //  IMEI: '356741396791015',
  // VIN: 'T123ZTZT396798869',
...
async function startEventTelemetry() {
  ...
  client.on('connect', () => {
    ...
    sendVehicleEvent(client, EventMessage, eTboxOperatingState, eTboxApplicationState, eTboxeSimState, IgnitionPropertyType);
    
    setInterval(() => {
      if (client.connected) sendVehicleEvent(client, EventMessage, eTboxOperatingState, eTboxApplicationState, eTboxeSimState, IgnitionPropertyType);
    }, 60000);
  });
...
function sendVehicleEvent(client, EventMessage, eTboxOperatingState, eTboxApplicationState, eTboxeSimState, IgnitionPropertyType) {
  sendCount++;
...
console.log("eventObj",eventObj);
  ...
  try {
    ...
    console.log(' TOPIC: /dongle/' + CONFIG.VIN + '/MQTTPROTOBUF/events/history');

    client.publish(`/dongle/${CONFIG.VIN}/MQTTPROTOBUF/events/history`, buf, { qos: 1 });
