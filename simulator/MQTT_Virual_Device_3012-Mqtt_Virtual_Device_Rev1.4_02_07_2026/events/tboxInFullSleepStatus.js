...
async function startEventTelemetry() {
  ...
  const TboxfullSleepType = root.lookupEnum('stla.cvip.jeep.TboxfullSleepType');
  ...
  console.log('TboxfullSleepType:', Object.keys(TboxfullSleepType.values));
  ...
  client.on('connect', () => {
    ...
    sendVehicleEvent(client, EventMessage, eTboxOperatingState, eTboxApplicationState, eTboxeSimState, TboxfullSleepType);
    
    setInterval(() => {
      if (client.connected) sendVehicleEvent(client, EventMessage, eTboxOperatingState, eTboxApplicationState, eTboxeSimState, TboxfullSleepType);
    }, 60000);
  });
...
function sendVehicleEvent(client, EventMessage, eTboxOperatingState, eTboxApplicationState, eTboxeSimState, TboxfullSleepType) {
  sendCount++;
  ...
  const full_sleep = TboxfullSleepType.values.full_sleep;
console.log("STARTSTART",full_sleep);
   ...
   // timeStamp: getTimestamp(),                         
    eventPayload: {                                    
      EventsData: [{
        ...
        TboxInFullSleepStatus: {
          TboxfullSleepState: full_sleep                           
        }
      }]
    }
  };
