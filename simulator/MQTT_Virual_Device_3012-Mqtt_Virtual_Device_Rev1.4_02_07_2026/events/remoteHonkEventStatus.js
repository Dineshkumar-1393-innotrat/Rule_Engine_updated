...
async function startEventTelemetry() {
  ...
  const RemoteHonkEventType = root.lookupEnum('stla.cvip.jeep.RemoteHonkEventType');
  ...
  console.log('RemoteHonkEventType:', Object.keys(RemoteHonkEventType.values));
  ...
  client.on('connect', () => {
    ...
    sendVehicleEvent(client, EventMessage, eTboxOperatingState, eTboxApplicationState, eTboxeSimState, RemoteHonkEventType);
    
    setInterval(() => {
      if (client.connected) sendVehicleEvent(client, EventMessage, eTboxOperatingState, eTboxApplicationState, eTboxeSimState, RemoteHonkEventType);
    }, 60000);
  });
...
function sendVehicleEvent(client, EventMessage, eTboxOperatingState, eTboxApplicationState, eTboxeSimState, RemoteHonkEventType) {
  sendCount++;
  ...
  const remoteHonk_on = RemoteHonkEventType.values.remoteHonk_on;
console.log("STARTSTART",remoteHonk_on);
   ...
   // timeStamp: getTimestamp(),                         
    eventPayload: {                                    
      EventsData: [{
        ...
        RemoteHonkEventStatus: {
          RemoteHonkEventState: remoteHonk_on                           
        }
      }]
    }
  };
