...
async function startEventTelemetry() {
  ...
  const VehicleMobilizationEventType = root.lookupEnum('stla.cvip.jeep.VehicleMobilizationEventType');
  ...
  console.log('VehicleMobilizationEventType:', Object.keys(VehicleMobilizationEventType.values));
  ...
  client.on('connect', () => {
    ...
    sendVehicleEvent(client, EventMessage, eTboxOperatingState, eTboxApplicationState, eTboxeSimState, VehicleMobilizationEventType);
    
    setInterval(() => {
      if (client.connected) sendVehicleEvent(client, EventMessage, eTboxOperatingState, eTboxApplicationState, eTboxeSimState, VehicleMobilizationEventType);
    }, 60000);
  });
...
function sendVehicleEvent(client, EventMessage, eTboxOperatingState, eTboxApplicationState, eTboxeSimState, VehicleMobilizationEventType) {
  sendCount++;
  ...
  const Mobilized = VehicleMobilizationEventType.values.Mobilized;
console.log("STARTSTART",Mobilized);
   ...
   // timeStamp: getTimestamp(),                         
    eventPayload: {                                    
      EventsData: [{
        ...
        VehicleMobilizationStatus: {
          VehicleMobilizationState: Mobilized                           
        }
      }]
    }
  };
