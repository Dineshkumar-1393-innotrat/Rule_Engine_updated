...
async function startEventTelemetry() {
  ...
  const RemoteDoorLockUnlockCommandEventType = root.lookupEnum('stla.cvip.jeep.RemoteDoorLockUnlockCommandEventType');
  ...
  console.log('RemoteDoorLockUnlockCommandEventType:', Object.keys(RemoteDoorLockUnlockCommandEventType.values));
  ...
  client.on('connect', () => {
    ...
    sendVehicleEvent(client, EventMessage, eTboxOperatingState, eTboxApplicationState, eTboxeSimState, RemoteDoorLockUnlockCommandEventType);
    
    setInterval(() => {
      if (client.connected) sendVehicleEvent(client, EventMessage, eTboxOperatingState, eTboxApplicationState, eTboxeSimState, RemoteDoorLockUnlockCommandEventType);
    }, 60000);
  });
...
function sendVehicleEvent(client, EventMessage, eTboxOperatingState, eTboxApplicationState, eTboxeSimState, RemoteDoorLockUnlockCommandEventType) {
  sendCount++;
  ...
  const request_Successful_All_Door_Unlocked = RemoteDoorLockUnlockCommandEventType.values.request_Successful_All_Door_Unlocked;
console.log("STARTSTART",request_Successful_All_Door_Unlocked);
   ...
   // timeStamp: getTimestamp(),                         
    eventPayload: {                                    
      EventsData: [{
        ...
        RemoteDoorLockUnlockCommandEventStatus: {
          RemoteDoorLockUnlockCommandEventState: request_Successful_All_Door_Unlocked                           
        }
      }]
    }
  };
