...
//     subtype: protoObjects.commandResponseSubType.values.RemoteDoorLockCommandResponse,
...
//       remoteDoorLockCommandResponsePayload: {
...
//  Send Speed Alert
async function sendSpeedAlert(client, vehicleSpeed) {
  sendCount++;
  ...
  try {
    ...
    console.log(`\n Sending Speed Alert #${sendCount}`);
    console.log("Vehicle Speed:", vehicleSpeed);

    client.publish(`/dongle/${CONFIG.VIN}/MQTTPROTOBUF/alerts`, buf, { qos: 1 });
...
async function start() {
  ...
  client.on('connect', () => {
    ...
    const topic = `/dongle/${CONFIG.VIN}/MQTTPROTOBUF/command`;
  ...
  client.on('message', (topic, message) => {
    ...
    try {
      ...
      const obj = protoObjects.CommandMessage.toObject(decoded, {
        longs: Number,
        enums: String,
        defaults: true
      });

      console.log(" DECODED:", JSON.stringify(obj, null, 2));

      // Extract correlationId
      dynamicCorrelationId = obj.correlationId;
      commandReceived = true;

      // Extract speed
      const commandSpeed =
        obj.commandPayload?.userDefinedSpeedCommandPayload?.speed;

      if (!commandSpeed) {
        console.log(" No speed found in command");
        return;
      }

      const speedLimit = Number(commandSpeed);
      console.log(" Speed Limit from Command:", speedLimit);

      // Simulated vehicle speed (you can change this)
      const vehicleSpeed = 120.5;

      console.log(" Current Vehicle Speed:", vehicleSpeed);

      // Compare and send alert
      if (vehicleSpeed > speedLimit) {
        console.log(" Speed exceeded! Sending alert...");
        sendSpeedAlert(client, vehicleSpeed);
      } else {
        console.log(" Speed within limit. No alert sent.");
      }
