...
// File Tile for Download Section
function FileTile({ name, type = "Plain Text", onClick }) {
  return (
    <motion.div
      whileHover={{ scale: 1.02, background: `${T.accent}08` }}
      whileTap={{ scale: 0.98 }}
      onClick={onClick}
      style={{
        background: "#1a1a1a",
...
// CVIP Preprod CA Certificates (Extracted from certificates.zip)
...
function CertificatePanel({ state, dispatch }) {
  ...
  const buildCsr = async (privateKey, publicKey) => {
    ...
    // Build subject attributes as per PDF Spec: C=IN, ST=State, L=City, O=Traxo, OU=Software, CN=...
  ...
  const handleGenerate = async () => {
    ...
    try {
      ...
      // Extract raw base64 from existingCsrPem (no headers, no newlines) 
      // matching the pattern seen in Collection 5.0
      ...
      const ts = Date.now(); // Milliseconds as per PDF (date +%s%3N)
      const joinPayload = {
        ...
        timestamp: ts, certificate: certRaw // certRaw is already base64 of DER
      };
      ...
      // Specific handling for Public Key Mismatch
      if (errorMsg.includes('Public key mismatch')) {
        setError('❌ Platform Error: Public key mismatch. This device (VIN/IMEI) is already registered with a different key pair on the platform. \n\nSUGGESTED ACTION: \n1. Try a different, unique VIN/IMEI.\n2. Or click the Trash icon to reset local keys and retry (only if you believe the local keys are incorrect).');
      } else {
        setError(errorMsg);
      }
        ...
        {open && (
          <motion.div key="cert-body" initial={{ height: 0, opacity: 0 }} animate={{ height: 'auto', opacity: 1 }} exit={{ height: 0, opacity: 0 }} transition={SPRINGS.panelCollapse} style={{ overflow: 'hidden' }}>
            ...
            <div style={{ display: 'flex', gap: 8, marginBottom: 10 }}>
              <PressButton onClick={handleGenerate} color={step === 4 ? T.pub : T.accent} disabled={step > 0 && step < 4 && step !== -1} style={{ flex: 4 }}>
                ...
                <>
                  <PressButton
                    onClick={() => {
                      ...
                      setTimeout(() => downloadFile(csrPem, 'device.csr'), 300);
                      setTimeout(() => downloadFile(certPem, 'device-cert.crt'), 600);
                      setTimeout(() => downloadFile(bundlePem, 'cvip-ca-bundle.crt'), 900);
                      setTimeout(() => downloadFile(deviceJoinPayload, 'device_join_payload.json'), 1200);
                    }}
                    color="#10b981"
                    style={{ flex: 2, padding: '7px 0', fontSize: 11 }}
                  ...
                  <PressButton
                    ...
                    color="#7c3aed"
                    style={{ flex: 2, padding: '7px 0', fontSize: 10 }}
              ...
              <PressButton 
                onClick={() => {
                  if (confirm('Are you sure? This will delete saved keys for this device. Use only if you want to generate a completely new key pair and deal with "Key Mismatch" errors.')) {
                    localStorage.removeItem(`pki_keys_${commonName}`);
                    dispatch({ type: 'ADD_TOAST', toast: { id: Date.now().toString(), message: 'Keys Reset', type: 'success' } });
                  }
                }} 
                color={T.error} 
            ...
            {step === 4 && (
              <div style={{ display: 'flex', flexDirection: 'column', gap: 12 }}>
                <div style={{ 
                  ...
                  <FileTile 
                    name="cvip-ca-bundle.crt" 
                    onClick={() => downloadFile(bundlePem, 'cvip-ca-bundle.crt')} 
                  />
                  <FileTile 
                    name="device.csr" 
                    onClick={() => downloadFile(csrPem, 'device.csr')} 
                  />
                  <FileTile 
                    name="device-cert.crt" 
                    onClick={() => downloadFile(certPem, 'device-cert.crt')} 
                  />
                  <FileTile 
                    name="device-key.key" 
                    onClick={() => downloadFile(privateKeyPem, 'device-key.key')} 
                  />
                </div>

                <PressButton color="#9333ea" style={{ width: '100%' }} onClick={() => downloadFile(deviceJoinPayload, 'device_join_payload.json')}>
                  ↓ Download deviceJoin Payload (.json)
                </PressButton>
...
// ─────────────────────────────────────────────
// SECTION 1B2 — CERTIFICATE VERIFIER
// ─────────────────────────────────────────────
function parsePemCert(pem) {
  try {
    // If it's a bundle, take only the first certificate for parsing metadata
    const match = pem.match(/-----BEGIN CERTIFICATE-----[\s\S]+?-----END CERTIFICATE-----/);
    const targetPem = match ? match[0] : pem;
    
    const b64 = targetPem.replace(/-----[^-]+-----/g, '').replace(/\s/g, '');
    const der  = Uint8Array.from(atob(b64), c => c.charCodeAt(0));
    // Very lightweight ASN.1 TBSCertificate reader — good enough for Subject/Validity
    const readLen = (view, off) => {
      if (view[off] < 0x80) return { len: view[off], next: off + 1 };
      const nb = view[off] & 0x7f;
      let l = 0;
      for (let i = 0; i < nb; i++) l = (l << 8) | view[off + 1 + i];
      return { len: l, next: off + 1 + nb };
    };
    const readSeq = (view, off) => {
      off++; // skip tag
      const { len, next } = readLen(view, off);
      return { start: next, len, end: next + len };
    };
    const v = der;
    // outer SEQUENCE
    let p = readSeq(v, 0);
    ...
    p = readSeq(v, p.start);
    const tbsStart = p.start;
    // skip version [0] EXPLICIT if present
    let cur = tbsStart;
    if (v[cur] === 0xa0) { const { len, next } = readLen(v, cur + 1); cur = next + len; }
    // serialNumber
    { const t = v[cur]; cur++; const { len, next } = readLen(v, cur); cur = next + len; }
    // signature AlgorithmIdentifier
    { cur++; const { len, next } = readLen(v, cur); cur = next + len; }
    // issuer Name
    const issuerStart = cur;
    { cur++; const { len, next } = readLen(v, cur); cur = next + len; }
    const issuerEnd = cur;
    // validity SEQUENCE
    { cur++; const { len, next } = readLen(v, cur);
      const vStart = next;
      // notBefore
      const nbTag = v[vStart]; cur = vStart + 1;
      const nbL = readLen(v, cur); const notBeforeRaw = new TextDecoder().decode(v.slice(nbL.next, nbL.next + nbL.len));
      cur = nbL.next + nbL.len;
      const naTag = v[cur]; cur++;
      const naL = readLen(v, cur); const notAfterRaw = new TextDecoder().decode(v.slice(naL.next, naL.next + naL.len));
      cur = naL.next + naL.len;
      // subject Name
      const subjectStart = cur;
      cur++; const subL = readLen(v, cur); cur = subL.next + subL.len;
      const subjectEnd = cur;
      // extract CN from Subject bytes
      const extractCN = (bytes) => {
        let cn = '';
        let i = 0;
        while (i < bytes.length) {
          if (bytes[i] === 0x31) { // SET
            i++; const sl = readLen(bytes, i); i = sl.next;
            if (bytes[i] === 0x30) { // SEQUENCE
              i++; const ql = readLen(bytes, i); i = ql.next;
              if (bytes[i] === 0x06) { // OID
                i++; const ol = readLen(bytes, i); i = ol.next;
                const oid = Array.from(bytes.slice(i, i + ol.len));
                i += ol.len;
                // CN OID = 55 04 03
                if (oid[0] === 0x55 && oid[1] === 0x04 && oid[2] === 0x03) {
                  i++; const vl = readLen(bytes, i); i = vl.next;
                  cn = new TextDecoder().decode(bytes.slice(i, i + vl.len));
                }
              }
            }
          } else i++;
      ...
      const subjectBytes = v.slice(subjectStart + 1, subjectEnd); // skip outer tag
      const issuerBytes  = v.slice(issuerStart  + 1, issuerEnd);
      const parseDate = (raw) => {
        // YYMMDDHHMMSSZ or YYYYMMDDHHMMSSZ
        const s = raw.replace('Z','');
        let y, m, d, hh, mm, ss;
        if (s.length === 12) { y = +('20'+s.slice(0,2)); m = +s.slice(2,4); d = +s.slice(4,6); hh = +s.slice(6,8); mm = +s.slice(8,10); ss = +s.slice(10,12); }
        else                  { y = +s.slice(0,4); m = +s.slice(4,6); d = +s.slice(6,8); hh = +s.slice(8,10); mm = +s.slice(10,12); ss = +s.slice(12,14); }
        return new Date(Date.UTC(y,m-1,d,hh,mm,ss));
      };
      return {
        cn: extractCN(subjectBytes) || '(not found)',
        subject: extractCN(subjectBytes),
        issuerCN: extractCN(issuerBytes) || '(not found)',
        notBefore: parseDate(notBeforeRaw),
        notAfter:  parseDate(notAfterRaw),
        isExpired: parseDate(notAfterRaw) < new Date(),
        isValid:   parseDate(notAfterRaw) >= new Date() && parseDate(notBeforeRaw) <= new Date(),
      };
    }
  } catch (e) {
    return null;
...
function CertVerifierPanel({ state, dispatch }) {
  const navigate = useNavigate();
  const [open, setOpen] = useState(false);
  const [certs, setCerts] = useState({
    deviceCert: { pem: '', info: null, filename: '' },
    deviceKey:  { pem: '', info: null, filename: '' },
    deviceCsr:  { pem: '', info: null, filename: '' },
    caBundle:   { pem: '', info: null, filename: '' },
  });
  ...
  const processFile = async (file, slotKey) => {
    const text = await readFile(file);
    // Handle cert-bundle.json
    if (file.name.endsWith('.json')) {
      try {
        const bundle = JSON.parse(text);
        const dc = { pem: bundle.certPem    || '', info: bundle.certPem    ? parsePemCert(bundle.certPem)    : null, filename: 'device-cert.crt (from bundle)' };
        const dk = { pem: bundle.privateKeyPem || '', info: null, filename: 'device-key.key (from bundle)' };
        const ds = { pem: bundle.csrPem        || '', info: null, filename: 'device.csr (from bundle)' };
        const ca = { pem: bundle.bundlePem   || '', info: bundle.bundlePem   ? parsePemCert(bundle.bundlePem.split('-----END CERTIFICATE-----')[0] + '-----END CERTIFICATE-----') : null, filename: 'cvip-ca-bundle.crt (from bundle)' };
        setCerts({ deviceCert: dc, deviceKey: dk, deviceCsr: ds, caBundle: ca });
        setBundleLoaded(true);
        dispatch({ type: 'ADD_TOAST', toast: { id: Date.now().toString(), message: '✓ cert-bundle.json loaded — all certs populated!', type: 'success' } });
      } catch { dispatch({ type: 'ADD_TOAST', toast: { id: Date.now().toString(), message: '❌ Invalid cert-bundle.json', type: 'error' } }); }
      return;
    }
    // PEM file
    const info = text.includes('CERTIFICATE') ? parsePemCert(text) : null;
    const slot = slotKey || (
      file.name.includes('bundle') || file.name.includes('ca') ? 'caBundle' :
      file.name.includes('key') ? 'deviceKey' : 
      file.name.includes('csr') ? 'deviceCsr' : 'deviceCert'
    );
    setCerts(prev => ({ ...prev, [slot]: { pem: text, info, filename: file.name } }));
  };

  const onDrop = (e, slotKey) => {
    e.preventDefault(); setDragOver(null);
    const file = e.dataTransfer.files[0];
    if (file) processFile(file, slotKey);
  };

  const UploadZone = ({ slotKey, label, icon }) => {
    const slot = certs[slotKey];
    const isOk  = (slotKey === 'deviceKey' || slotKey === 'deviceCsr') ? !!slot.pem : slot.info?.isValid;
    const isBad = (slotKey !== 'deviceKey' && slotKey !== 'deviceCsr') && slot.info && !slot.info.isValid;
    ...
    return (
      <div
        ...
        style={{
          border: `2px dashed ${isDrag ? T.accent : isOk ? '#10b981' : isBad ? T.error : T.border}`,
          borderRadius: 8, padding: '10px 8px', textAlign: 'center',
          background: isDrag ? `${T.accent}11` : isOk ? '#10b98111' : isBad ? `${T.error}11` : T.bgPrimary,
          cursor: 'pointer', transition: 'all 0.2s', position: 'relative',
        }}
        onClick={() => { const inp = document.createElement('input'); inp.type='file'; inp.onchange = e => processFile(e.target.files[0], slotKey); inp.click(); }}
      >
        <div style={{ fontSize: 18, marginBottom: 4 }}>{isOk ? '✅' : isBad ? '❌' : icon}</div>
        <div style={{ fontSize: 10, fontWeight: 700, color: T.textPri, fontFamily: T.mono }}>{label}</div>
        {slot.filename
          ? <div style={{ fontSize: 9, color: isOk ? '#10b981' : T.textMut, marginTop: 2, wordBreak: 'break-all' }}>{slot.filename}</div>
          : <div style={{ fontSize: 9, color: T.textMut, marginTop: 2 }}>Drop or click</div>
        }
        {slot.info && (
          <div style={{ marginTop: 6, textAlign: 'left', fontFamily: T.mono, fontSize: 9, lineHeight: 1.6 }}>
            <div><span style={{ color: T.textMut }}>CN:</span> <span style={{ color: T.accent, fontWeight: 700 }}>{slot.info.cn}</span></div>
            <div><span style={{ color: T.textMut }}>Issuer:</span> <span style={{ color: T.textPri }}>{slot.info.issuerCN}</span></div>
            <div><span style={{ color: T.textMut }}>Valid From:</span> <span style={{ color: T.textPri }}>{slot.info.notBefore?.toLocaleDateString()}</span></div>
            <div>
              <span style={{ color: T.textMut }}>Expires:</span>{' '}
              <span style={{ color: slot.info.isExpired ? T.error : '#10b981', fontWeight: 700 }}>
                {slot.info.notAfter?.toLocaleDateString()} {slot.info.isExpired ? '⚠ EXPIRED' : '✓'}
              </span>
            </div>
          </div>
        )}
  ...
  const allValid = certs.deviceCert.info?.isValid && !!certs.deviceKey.pem && !!certs.deviceCsr.pem && certs.caBundle.info?.isValid;
  ...
  return (
    <Card>
      <div
        ...
        onClick={() => setOpen(o => !o)}
      >
        <SectionHeader>🛡️ Certificate Verifier</SectionHeader>
        <div style={{ display: 'flex', gap: 8, alignItems: 'center' }}>
          {allValid && <span style={{ fontSize: 10, color: '#10b981', fontWeight: 700, fontFamily: T.mono }}>ALL VALID ✓</span>}
          <motion.span animate={{ rotate: open ? 180 : 0 }} transition={SPRINGS.panelCollapse} style={{ color: T.accent, fontSize: 16, marginTop: -6 }}>▼</motion.span>
        </div>
      </div>
      <AnimatePresence initial={false}>
        {open && (
          <motion.div key="cv-body" initial={{ height: 0, opacity: 0 }} animate={{ height: 'auto', opacity: 1 }} exit={{ height: 0, opacity: 0 }} transition={SPRINGS.panelCollapse} style={{ overflow: 'hidden' }}>
            <div style={{ fontSize: 10, color: T.textMut, marginBottom: 10, fontFamily: T.mono, background: `${T.accent}0d`, padding: '6px 8px', borderRadius: 6, border: `1px solid ${T.accent}22` }}>
              Upload your downloaded cert files one by one, or drop <strong>cert-bundle.json</strong> to auto-populate all.
            </div>

            {/* cert-bundle.json drop zone */}
            <div
              onDragOver={e => { e.preventDefault(); setDragOver('bundle'); }}
              onDragLeave={() => setDragOver(null)}
              onDrop={e => onDrop(e, null)}
              onClick={() => { const inp = document.createElement('input'); inp.type='file'; inp.accept='.json'; inp.onchange = e => processFile(e.target.files[0], null); inp.click(); }}
              style={{
                border: `2px dashed ${dragOver === 'bundle' ? '#7c3aed' : bundleLoaded ? '#10b981' : T.border}`,
                borderRadius: 8, padding: '10px', textAlign: 'center', marginBottom: 10,
                background: bundleLoaded ? '#10b98111' : dragOver === 'bundle' ? '#7c3aed11' : T.bgPrimary,
                cursor: 'pointer', transition: 'all 0.2s',
              }}
            >
              <div style={{ fontSize: 16 }}>{bundleLoaded ? '✅' : '📦'}</div>
              <div style={{ fontSize: 10, fontWeight: 700, color: '#7c3aed', fontFamily: T.mono }}>cert-bundle.json (All-in-one)</div>
              <div style={{ fontSize: 9, color: T.textMut }}>
                {bundleLoaded ? 'Bundle loaded — all certs populated' : 'Drop exported cert-bundle.json here to auto-fill all fields'}
              </div>
            </div>

            <div style={{ display: 'flex', flexDirection: 'column', gap: 8, marginBottom: 10 }}>
              <UploadZone slotKey="deviceCert" label="device-cert.crt" icon="📄" />
              <UploadZone slotKey="deviceKey"  label="device-key.key"  icon="🔑" />
              <UploadZone slotKey="deviceCsr"  label="device.csr"      icon="📝" />
              <UploadZone slotKey="caBundle"   label="cvip-ca-bundle.crt" icon="🏛️" />
            </div>
            {allValid && (
              <div style={{ display: 'flex', flexDirection: 'column', gap: 8, marginBottom: 10 }}>
                <div style={{ fontSize: 10, fontWeight: 700, color: T.textPri, fontFamily: T.mono, marginBottom: 2 }}>
                  🚀 Simulate in UI
                </div>
                ...
                <PressButton
                  ...
                  onClick={() => {
                    const payload = {
                      ...
                      certificate: certs.deviceCert.pem ? btoa(certs.deviceCert.pem) : '',
                    };
                    ...
                    dispatch({ type: 'ADD_TOAST', toast: { id: Date.now().toString(), message: '✓ Device Join simulated — status: CONNECTED', type: 'success' } });
                  }}
                >
                  ▶ Simulate Device Join
                </PressButton>

                {/* Step 2: Vehicle Telemetry */}
                <PressButton
                  color="#10b981"
                  style={{ width: '100%' }}
                  onClick={() => {
                    ...
                    const telPayload = {
                      ...
                      canDataPayload: {
                        canData: {
                          canDataPacket: [{ identifier: 854, dlc: 8, cData: [{ pdu: '0x6400000000000000', timestamp: ts }] }]
                        }
                      }
                    };
                    dispatch({ type: 'SET_TELEMETRY', key: 'fuelLevel', value: 100 });
                    dispatch({ type: 'SET_TELEMETRY', key: 'engineSpeed', value: 2800 });
                    dispatch({ type: 'SET_TELEMETRY', key: 'engineWaterTemp', value: 90 });
                    dispatch({ type: 'SET_TELEMETRY', key: 'batteryVoltage', value: 12.6 });
                    ...
                    dispatch({
                      ...
                      entry: {
                        ...
                        status: 'SUCCESS',
                        payloadJson: telPayload,
                        expanded: false,
                      }
                    });
                    dispatch({ type: 'ADD_TOAST', toast: { id: Date.now().toString(), message: '📡 Vehicle telemetry published!', type: 'success' } });
                  }}
                >
                  📡 Simulate Vehicle Telemetry
                </PressButton>

                {/* Step 3: Device Join Response (Subscribe) */}
                <PressButton
                  color="#9333ea"
                  style={{ width: '100%' }}
                  onClick={() => {
                    dispatch({
                      ...
                      entry: {
                        ...
                        timestamp: new Date().toISOString(),
                        direction: 'SUB',
                        messageType: 'deviceJoinedRsp',
                        topic: `/dongle/${state.config.vin}/MQTTPROTOBUF/deviceJoinedRsp`,
                        status: 'SUCCESS',
                        payloadJson: { status: 'SUCCESS', vehicleId: state.config.vin, message: 'Device registered successfully' },
                        expanded: false,
                      }
                    });
                    dispatch({ type: 'ADD_TOAST', toast: { id: Date.now().toString(), message: '📥 Simulated platform response received', type: 'success' } });
                  }}
                >
                  📥 Simulate Platform Response
                </PressButton>

                {/* Step 4: Link to Official Backend Check */}
                <PressButton
                  color="#d97706"
                    ...
                    navigate('/certificate-verification', {
                      state: {
                        autoFill: {
                          ...
                          imeiNo: state.config.imei,
                          TboxSerialNum: state.config.tboxSerial,
                          certificate: certs.deviceCert.pem ? certs.deviceCert.pem.replace(/-----[^-]+-----/g, '').replace(/\s/g, '') : ''
                        }
                      }
                    });
                  }}
                >
                  🛡️ Run Official Backend Check
                </PressButton>
              </div>
            )}

            {allValid && (
              <div style={{ background: '#10b98111', border: '1px solid #10b98133', borderRadius: 8, padding: 10, fontFamily: T.mono, fontSize: 10, color: '#10b981' }}>
                <div style={{ fontWeight: 700, marginBottom: 4 }}>✅ All certificates verified and valid!</div>
                <div>Device CN: <strong>{certs.deviceCert.info?.cn}</strong></div>
                <div>Expires: <strong>{certs.deviceCert.info?.notAfter?.toLocaleDateString()}</strong></div>
                <div style={{ marginTop: 4, color: T.textMut }}>Run <code>node sync-certs.js</code> in the simulator folder to push these certs and start the MQTT connection.</div>
              </div>
            )}
          </motion.div>
        )}
...
// ─────────────────────────────────────────────
// SECTION 1C — PLATFORM STATE MANAGEMENT
// ─────────────────────────────────────────────
function PlatformStatePanel({ state, dispatch }) {
  const [open, setOpen] = useState(false);
  ...
  const handleFetchState = async () => {
    ...
    try {
      const { TraxoApi } = await import('../../utils/TraxoApi');
      const resp = await TraxoApi.getDeviceStateByVin(vin);
      setCurrentState(resp);
      dispatch({ type: "ADD_TOAST", toast: { id: uuidv4(), message: "✓ Platform state fetched", type: "success" } });
    } catch (e) {
      if (e.response?.status === 404) {
        setCurrentState({ statusCode: 404 });
        dispatch({ type: "ADD_TOAST", toast: { id: uuidv4(), message: "Device not found on platform (404)", type: "error" } });
      } else {
        dispatch({ type: "ADD_TOAST", toast: { id: uuidv4(), message: `❌ Fetch failed: ${e.message}`, type: "error" } });
      }
    } finally {
      setLoading(false);
    }
  ...
  const handleAutoProvision = async () => {
    ...
    try {
      const { TraxoApi } = await import('../../utils/TraxoApi');
      const { imei, vin } = state.config;
      
      // Step 1: IMEI Provisioning
      const imeiBlob = new Blob([imei], { type: 'text/csv' });
      const imeiFile = new File([imeiBlob], 'imei.csv', { type: 'text/csv' });
      await TraxoApi.bulkImeiUpload(imeiFile);
      
      // Step 2: Supplier Feed
      const header = "Dongle_SN,IMEI,MSISDN,ICCID,eUICCID,HW_part_number,NAD_SW_version,MCU_SW_version,encryptionKeyVersion,signingKeyVersion,plantManufacturingCountryCode,Plant_Manufactured_Date,countrycode,regioncode";
      const row = `${vin},${imei},1234567890,1234567890,1234567890,PART1,1.0,1.0,1,1,1,20240101,1,1`;
      const supplierBlob = new Blob([`${header}\n${row}`], { type: 'text/csv' });
      const supplierFile = new File([supplierBlob], 'supplier.csv', { type: 'text/csv' });
      await TraxoApi.bulkSupplierFeed(supplierFile);
      
      dispatch({ type: "ADD_TOAST", toast: { id: uuidv4(), message: "✓ Device Provisioned Successfully", type: "success" } });
      handleFetchState();
    } catch (e) {
      dispatch({ type: "ADD_TOAST", toast: { id: uuidv4(), message: `❌ Provisioning failed: ${e.message}`, type: "error" } });
    } finally {
      setLoading(false);
    }
  };
  ...
  const handleUpdate = async (target) => {
    ...
    try {
      const { TraxoApi } = await import('../../utils/TraxoApi');
      const resp = await TraxoApi.updateTboxState(vin, target);
      dispatch({ type: "ADD_TOAST", toast: { id: uuidv4(), message: `✓ Platform state updated to ${target}`, type: "success" } });
      dispatch({
        type: "ADD_LOG",
        entry: {
          id: uuidv4(),
          timestamp: new Date().toISOString(),
          direction: "API",
          messageType: "tboxstateupdate",
          topic: "/concurrentcommands/vinno",
          status: "SUCCESS",
          payloadJson: resp,
          expanded: false
        }
      ...
      dispatch({ type: "ADD_TOAST", toast: { id: uuidv4(), message: `❌ Update failed: ${e.message}`, type: "error" } });
    } finally {
      setLoading(false);
    }
  };
  ...
  const handleReset = async () => {
    ...
    try {
      ...
      dispatch({ type: "ADD_TOAST", toast: { id: uuidv4(), message: "✓ Device state reset successfully", type: "success" } });
    } catch (e) {
      dispatch({ type: "ADD_TOAST", toast: { id: uuidv4(), message: `❌ Reset failed: ${e.message}`, type: "error" } });
    } finally {
      setLoading(false);
    }
  };
  ...
  return (
    <Card>
      <div
        ...
        onClick={() => setOpen(o => !o)}
      >
        <SectionHeader>🚀 Platform State</SectionHeader>
        <motion.span animate={{ rotate: open ? 180 : 0 }} transition={SPRINGS.panelCollapse} style={{ color: T.accent, fontSize: 16, marginTop: -6 }}>▼</motion.span>
      </div>

      <AnimatePresence initial={false}>
        {open && (
          <motion.div key="platform-body" initial={{ height: 0, opacity: 0 }} animate={{ height: 'auto', opacity: 1 }} exit={{ height: 0, opacity: 0 }} transition={SPRINGS.panelCollapse} style={{ overflow: 'hidden' }}>
            <div style={{ marginBottom: 14 }}>
              <Label>Platform Status</Label>
              <PressButton onClick={handleFetchState} color={T.sub} style={{ width: '100%', marginBottom: 8 }} disabled={loading}>
                {loading ? '⟳ Fetching...' : '🔍 Check Current State'}
              </PressButton>
              {currentState && (
                <div style={{ padding: 8, background: T.bgPrimary, borderRadius: 6, border: `1px solid ${T.border}`, fontSize: 10, fontFamily: T.mono }}>
                  <div style={{ display: 'flex', justifyContent: 'space-between' }}>
                    <span style={{ color: T.textMut }}>State:</span>
                    <span style={{ color: T.pub }}>{currentState.tboxOperatingState || 'N/A'}</span>
                  </div>
                  <div style={{ display: 'flex', justifyContent: 'space-between', marginTop: 4 }}>
                    <span style={{ color: T.textMut }}>App State:</span>
                    <span style={{ color: T.pub }}>{currentState.tboxApplicationState || 'N/A'}</span>
                  </div>
                </div>
              )}
              {currentState?.statusCode === 404 && (
                <div style={{ marginTop: 8 }}>
                  <Badge color={T.error} style={{ width: '100%', padding: '6px 0', marginBottom: 6 }}>Device Not Provisioned</Badge>
                  <PressButton onClick={handleAutoProvision} color="#3b82f6" style={{ width: '100%', fontSize: 11 }} disabled={loading}>
                    {loading ? '⟳ Provisioning...' : '✨ Auto-Provision this Device'}
                  </PressButton>
                  <div style={{ fontSize: 9, color: T.textMut, marginTop: 4, textAlign: 'center' }}>
                    Registers IMEI and VIN on Preprod Platform
                  </div>
                </div>
              )}
            </div>
            ...
            <div style={{ marginBottom: 14 }}>
              <Label>Update TBOX State (CVIP API)</Label>
              <div style={{ display: 'flex', gap: 6 }}>
                <PressButton onClick={() => handleUpdate("CUSTOMER")} color={T.pub} style={{ flex: 1 }} disabled={loading}>CUSTOMER</PressButton>
                <PressButton onClick={() => handleUpdate("FACTORY")} color={T.accent} style={{ flex: 1 }} disabled={loading}>FACTORY</PressButton>
              </div>
            </div>

            <div>
              <Label>System Reset (AWS API)</Label>
              <PressButton onClick={handleReset} color={T.error} style={{ width: '100%' }} disabled={loading}>
                {loading ? '⟳ Processing...' : '⚠ Reset Device State'}
              </PressButton>
              <div style={{ fontSize: 9, color: T.textMut, marginTop: 6, fontFamily: T.mono }}>
                ↑ Clears platform cache and resets device join status
              </div>
            </div>
          </motion.div>
        )}
...
function VehicleTelemetryTab({ state, dispatch, onPublish }) {
  ...
  const telTypes = [
    ...
    { value: "engineSpeed",     label: "Engine Speed", unit: "RPM" },
    { value: "engineWaterTemp", label: "Engine Water Temp", unit: "°C" },
    { value: "batteryVoltage",  label: "Battery Voltage", unit: "V" },
    { value: "odometer",        label: "Odometer", unit: "km" },
  ];
  ...
  const handlePublish = useCallback(() => {
    ...
    const mockPayload = {
      ...
      canDataPayload: {
        canData: {
          canDataPacket: [{
            identifier: telType === "fuelLevel" ? 854 : 100,
            dlc: 8,
...
function LocationTelemetryTab({ state, dispatch, onPublish }) {
  ...
  const handlePublish = () => {
    ...
    dispatch({ type: "SET_TELEMETRY_BULK", payload: {
...
function AutoDriveButton({ state, dispatch, onPublish }) {
  ...
  const startAutoDrive = useCallback(() => {
    ...
    let lat = 13.084534, lng = 80.270718, speed = 60, fuel = 80;
  ...
  }, [state.config.vin, dispatch, onPublish]);
    ...
    <div style={{
      ...
      <div style={{
        ...
        <div style={{
          ...
          <CertVerifierPanel    state={state} dispatch={dispatch} />
          <PlatformStatePanel    state={state} dispatch={dispatch} />
